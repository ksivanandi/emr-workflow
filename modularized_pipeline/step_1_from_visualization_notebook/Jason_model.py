...
class Model:
    ...
    def predict(self, dataset, prediction_directory=None, groundtruth_directory=None):
        ...
        if isinstance(dataset, str):
            ...
            #display entity jason
            for entity_name, start, stop, end in annotations.get_entity_annotations():
                #span = Span(doc, start, end, label=medacy_pipeline.vocab.strings[entity_name])
                span = Span(doc, start, end, label=int(33333333))
                for token in span:
